...
#Vectorized Dataframes to perform operations on columns
def clean_tweets(lst):
    ...
    lst=np.vectorize(cleanData)(lst)
    return lst
df=pd.read_csv('OutputStreaming.csv')
df['Text']=clean_tweets(df['Text'])
df['Location']=df['Location'].astype(str)
df['Location']=clean_tweets(df['Location'])
#Cleaned data is lodaed into CSV
df.to_csv("tweet_cleaned.csv",index=None,header=True)
#Cleaned data is lodaed into JSON
df.to_csv("tweet_cleaned.txt",index=None,header=True)
